...
func isIndexManifest(metatype string) bool {
	switch metatype {
...
func GetImagePlatform(ctx context.Context, provider content.Provider, desc ocispec.Descriptor, platform platforms.MatchComparer) (*ocispec.Platform, error) {
	if !isIndexManifest(desc.MediaType) {
		return nil, nil
	}
	...
	sort.SliceStable(manifests, func(i, j int) bool {
		...
		return platform.Less(*manifests[i].Platform, *manifests[j].Platform)
	})

	return manifests[0].Platform, nil
}
