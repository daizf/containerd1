...
func readImage(image *images.Image, bkt *bolt.Bucket) error {
	...
	return tbkt.ForEach(func(k, v []byte) error {
		...
		case string(bucketKeyPlatform):
			image.Target.Platform, _ = decodePlatform(v)
		}
		return nil
	})
}
...
func writeImage(bkt *bolt.Bucket, image *images.Image) error {
	...
	platformEncoded, err := encodePlatform(image.Target.Platform)
	if err != nil {
		return err
	}
	...
	for _, v := range [][2][]byte{
		...
		{bucketKeyPlatform, platformEncoded},
	} {
...
func encodePlatform(platform *ocispec.Platform) ([]byte, error) {
	if platform == nil {
		return nil, nil
	}
...
func decodePlatform(b []byte) (*ocispec.Platform, error) {
	if b == nil {
		return nil, nil
	}
