...
func (c *criService) CreateContainer(ctx context.Context, r *runtime.CreateContainerRequest) (_ *runtime.CreateContainerResponse, retErr error) {
	...
	if value, ok := r.Config.Labels["eci/setup-net-class-id"]; ok {
		classID, err := strconv.ParseUint(value, 10, 64)
		if err != nil {
			return nil, fmt.Errorf("invalid net class ID:%w", err)
		}

		clsID := uint32(classID)
		log.G(ctx).Debugf("setup net class ID %d for container %q", clsID, id)
		if spec.Linux.Resources.Network == nil {
			spec.Linux.Resources.Network = &runtimespec.LinuxNetwork{ClassID: &clsID}
		} else {
			spec.Linux.Resources.Network.ClassID = &clsID
		}
	}
	...
	opts := []containerd.NewContainerOpts{
		containerd.WithSnapshotter(snapshotter.GetSnapshotter(ctx, c.config.ContainerdConfig.Snapshotter)),
	...
	var snapOpts containerd.NewContainerOpts
	// pause container do not use custom snapshotter
	if diskQuota, ok := r.SandboxConfig.Annotations[snapshotter.AnnotationQuotaoverlayfs]; ok && diskQuota != "" {
		snapOpts = customopts.WithNewSnapshotAndQuotaLabels(id, containerdImage, diskQuota)
	} else {
		snapOpts = customopts.WithNewSnapshot(id, containerdImage, snapshotterOpt)
	}
	opts = append(opts, snapOpts)
