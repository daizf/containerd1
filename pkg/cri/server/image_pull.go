...
import (
	...
	"github.com/containerd/containerd/pkg/cri/platforms"
)
...
const (
	EciImageType         = "k8s.aliyun.com/eci-image-type"
	EciImageTypeLabelKey = "eci.image.type"
)
...
func (c *criService) PullImage(ctx context.Context, r *runtime.PullImageRequest) (*runtime.PullImageResponse, error) {
	...
	ctx = snapshotter.ChooseSnapshotter(ctx, r.SandboxConfig)
	log.G(ctx).Info("PullImage with ChooseSnapshotter:", ctx.Value(snapshotter.Key()))
	...
	// get platform matcher from containerd config and pod annotation
	// 1. pod created with nydus hint annotation
	// 2.  containerd started with nydus snapshotter,
	platformMatcher = platforms.GetPlatformFromPodAnnotation(c.config, r.SandboxConfig)


	var host string
	username, secret, err := ParseAuth(auth, "")
	if err != nil {
		return nil, err
	}
	if auth != nil {
	...
	pullOpts := []containerd.RemoteOpt{
		...
		containerd.WithPullSnapshotter(
			snapshotter.GetSnapshotter(ctx, c.config.ContainerdConfig.Snapshotter),
			snapshots.WithPullAuth(host, username, secret),
		),
	...
	if r.SandboxConfig != nil && r.SandboxConfig.Annotations != nil {
		if imageType, exist := r.SandboxConfig.Annotations[EciImageType]; exist && imageType != "" {
			ctx = context.WithValue(ctx, EciImageType, imageType)
			pullOpts = append(pullOpts, containerd.WithPullLabel(EciImageTypeLabelKey, imageType))
		}
	}

	if err := c.checkImagePullDependency(ref); err != nil {
		log.G(ctx).Warnf("err check image dependency %v", err)
	}
...
func (c *criService) createImageReference(ctx context.Context, name string, desc imagespec.Descriptor) error {
	...
	// add image type label for imc
	imageType := ctx.Value(EciImageType)
	if val, ok := imageType.(string); ok {
		img.Labels[EciImageTypeLabelKey] = val
	}
...
func (c *criService) registryHosts(ctx context.Context, auth *runtime.AuthConfig) docker.RegistryHosts {
	...
	return func(host string) ([]docker.RegistryHost, error) {
		...
		for _, e := range endpoints {
			...
			if config.PlainHttp != nil && *config.PlainHttp == true {
				u.Scheme = "http"
			}
...
// checkImagePullDependency check image pull dependency,but not block image pull
func (c *criService) checkImagePullDependency(image string) error {

	// check p2p start
	if UseP2P(image) {
		p2pRegistry := strings.Split(image, "/")[0]
		registryAndPort := strings.Split(p2pRegistry, ":")
		p2pRegistryName := registryAndPort[0]
		p2pRegistryPort := "80"
		if len(registryAndPort) > 1 {
			p2pRegistryPort = registryAndPort[1]
		}
		c.config.Registry.RLock()
		if _, ok := c.config.Registry.Mirrors[p2pRegistry]; ok {
			//already set mirror skip
			c.config.Registry.RUnlock()
			goto SUCCESS
		}
		c.config.Registry.RUnlock()

		// not set image mirror
		if err := util.CheckP2PAgentStarted(p2pRegistryPort); err != nil {
			normalRegistry := strings.ReplaceAll(p2pRegistryName, ".distributed", "")
			c.config.Registry.Lock()
			c.config.Registry.Mirrors[p2pRegistry] = criconfig.Mirror{Endpoints: []string{normalRegistry}}
			c.config.Registry.Unlock()
			return fmt.Errorf("wait p2p agent start failed %v fallback to normal download", err)
		}
	}
...
// UseP2P whether use p2p accelerate
func UseP2P(image string) bool {
	p2pRegex := regexp.MustCompile(`[0-9a-z\-]+\.distributed\.[0-9a-z\-]+\.cr\.aliyuncs\.com`)
	return p2pRegex.MatchString(image)
}
