...
func (m *matcher) compareOSFeature(platform specs.Platform) bool {
	if m.OSVersion != platform.OSVersion {
		return false
	}
	if len(m.OSFeatures) != len(platform.OSFeatures) {
		return false
	}
	for i := range m.OSFeatures {
		if m.OSFeatures[i] != platform.OSFeatures[i] {
			return false
		}
	}
	return true
