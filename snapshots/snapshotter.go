...
const (
	SnapshotterPullAuthHostKey = "containerd.io/snapshot/pullauthhost"
	SnapshotterPullUsernameKey = "containerd.io/snapshot/pullusername"
	SnapshotterPullSecretKey   = "containerd.io/snapshot/pullsecret"
)
...
// WithPullAuth appends pull auth info via lables to a created snapshot
func WithPullAuth(host, username, secret string) Opt {
	return func(info *Info) error {
		if info.Labels == nil {
			info.Labels = make(map[string]string)
		}

		info.Labels[SnapshotterPullAuthHostKey] = host
		info.Labels[SnapshotterPullUsernameKey] = username
		info.Labels[SnapshotterPullSecretKey] = secret
